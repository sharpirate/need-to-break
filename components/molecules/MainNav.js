  ...
  return (
    <nav className="flex w-full justify-between 732:p-32 732:pb-0 932:p-48 932:pb-0">
      ...
      <ul className={`p-12 420:px-24 bg-primary-600 flex justify-between w-full 732:w-auto 732:p-0 732:bg-transparent 732:inline-grid 732:grid-cols-4`}>
        ...
        <SettingsButton handleClick={() => setModalIsOpen(true)} type={types.mobile} />
      </ul>
      <SettingsButton handleClick={() => setModalIsOpen(true)} type={types.desktop} />
...
function SettingsButton({ invisible, type, handleClick }) {
  let styleType = '';

  switch (type) {
    case types.desktop:
      styleType = 'hidden 732:block group outline-none';
      break;
      case types.mobile:

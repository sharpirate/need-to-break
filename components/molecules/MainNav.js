...
function MainNav({ tabs, active, handlePageChange }) {
  ...
  return (
    <nav className="flex w-full justify-between 732:p-32 732:pb-0 932:p-48 932:pb-0">
      <NavButton handleClick={() => setLogoutIsOpen(true)} forType={forTypes.desktop} type={types.logout} />
      <ul className={`p-12 420:px-16 bg-primary-600 flex justify-between w-full 732:w-auto 732:p-0 732:bg-transparent 732:inline-grid 732:grid-cols-4`}>
        <NavButton handleClick={() => setLogoutIsOpen(true)} forType={forTypes.mobile} type={types.logout} />
        {tabs.map((tab, index) => (
          <Tab
            key={tab.name}
        ...
        <NavButton handleClick={() => setSettingsIsOpen(true)} forType={forTypes.mobile} type={types.settings} />
      </ul>
      <NavButton handleClick={() => setSettingsIsOpen(true)} forType={forTypes.desktop} type={types.settings} />

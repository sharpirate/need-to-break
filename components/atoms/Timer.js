...
function Timer({ type }) {
  ...
  switch (type) {
    case intervalTypes.work:
      ...
      ringColor = 'text-primary-300';
      break;
    case intervalTypes.break:
      primaryColor = 'text-primary-400';
      accentColor = 'text-primary-600';
      ringColor = 'text-primary-300';
      break;
    case intervalTypes.blocked:
      primaryColor = 'text-blocked-500';
      accentColor = 'text-blocked-600';
      ringColor = 'text-blocked-300';
      break;
    case intervalTypes.floating:
      primaryColor = 'text-blocked-400';
      accentColor = 'text-blocked-600';
      ringColor = 'text-blocked-300';
      break;
    default:
  ...
  return (
    <div className="relative inline-block">
      ...
      <div className="absolute flex flex-col justify-center align-middle select-none text-center w-100 420:w-120 h-100 420:h-120 top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2">
        <span className={`font-display text-50 420:text-64 tracking-2 ${accentColor}`}>45</span>
        <span className={`absolute left-1/2 bottom-0 transform -translate-x-1/2 -translate-y-3/7 font-bold text-16 420:text-20 tracking-2 ${accentColor}`}>MIN</span>
      </div>
    </div>
  );
}

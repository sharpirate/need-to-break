...
import { setStoredLocalStorage } from "./localStorageUtil";
import { useSettings } from "../context/Settings";
import { get12HourTime } from "./timeUtil";
...
export function blueprintToStored(blueprint) {
  const { workDuration, breakDuration, startTime, duration, startWith } =
    blueprint;
  ...
  do {
    ...
    if (i === 0) {
      ...
      type =
        intervals[i - 1].type === intervalTypes.work
          ? intervalTypes.break
          : intervalTypes.work;
    }

    const intervalDuration =
      type === intervalTypes.work ? workDuration : breakDuration;
...
export function storedToTimeline(stored) {
  ...
  const parsedStartTime =
    typeof startTime === "string" ? parseStartTime(startTime) : startTime;
...
export function getDetails(blueprint) {
  ...
  const type =
    typeof blueprint.startTime === "number" ? "Flexible" : "Full Time";

  const startTime = isNaN(blueprint.startTime)
    ? parseStartTime(blueprint.startTime)
    : blueprint.startTime;

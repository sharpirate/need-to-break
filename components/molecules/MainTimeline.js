...
function MainTimeline() {
  ...
  function tick(now) {
    // get timer time left
    const endTimestamp =
      activeInterval.timestamp + activeInterval.duration * 1000;
    setTimeLeft(Math.round((endTimestamp - now) / 1000));

    // get timeline progress
    const timelineProgress =
      (Math.round((now - timeline.startTime) / 1000) / timeline.duration) * 100;
  ...
  function showNotificationOnIntervalEnd(nextInterval) {
    if (!nextInterval) {
      ...
      const title =
        nextInterval.type === intervalTypes.work ? "Work Time!" : "Break Time!";
  ...
  function handleRestart(e) {
    ...
    if (useSmartRestart) {
      ...
      closestBlock =
        timeToLeftBlock < timeToRightBlock ? leftBlock : rightBlock;
    }
    ...
    if (useSmartRestart) {
      if (activeInterval.type === restartType) {
        const intervalDuration =
          restartType === intervalTypes.work ? workDuration : breakDuration;
        intervalEnd = closestBlock + intervalDuration * 1000;
        // invert start block
        startBlock =
          restartType === intervalTypes.work
            ? intervalTypes.break
            : intervalTypes.work;
      } else {
    ...
    const prevIntervals = intervalsCopy
      .filter((interval, i) => i <= activeIntervalIndex)
      .map((interval) => ({
        type: interval.type,
    ...
    setStoredLocalStorage(newStored);
    startTimeline();
  }
  ...
  if (activeInterval) {
    ...
    timeLabel =
      activeInterval.type === intervalTypes.starting
        ? endTime
        : `${startTime} - ${endTime}`;
  }

  function renderNoTimeline() {
  ...
  return readyToShow ? (
    <div className="w-full flex flex-col justify-center items-center text-center bg-white rounded-8 py-16 px-32 420:py-24 420:px-48 932:py-32">
      ...
      {/* Restart Block */}
      <form
        className="flex flex-col justify-center items-center mb-32 420:mb-48"
        onSubmit={(e) => {
          e.preventDefault();
      ...
      >
        ...
        >
          <div className="mt-16 420:mt-24">
            <Label size={labelTypes.large} as={labelTypes.h1}>
              Restart
            </Label>
            ...
            <div className="flex justify-center items-center gap-16 420:gap-24 mb-24 420:mb-32">
              <RadioButton
                isChecked={restartType === intervalTypes.work}
                handleChange={handleRestartChange}
                value={intervalTypes.work}
                name="restartType"
                id="work"
                label="Work"
              />
              <RadioButton
                isChecked={restartType === intervalTypes.break}
                handleChange={handleRestartChange}
                value={intervalTypes.break}
                name="restartType"
                id="break"
                label="Break"
              />
            </div>
            <Button isSubmit type={buttonTypes.primary}>
              Restart
            </Button>
          </div>
        </ViewMoreLess>
      ...
      <div className="mb-16 420:mb-24">
        <Label size={labelTypes.large} as={labelTypes.h1} capitalize>
          {activeInterval.type}
        </Label>
      ...
      >
        <div className="flex flex-col mt-32 420:mt-48 932:mt-0 w-full">
          <Timeline timeline={timeline} progress={progress} />
        </div>
        <div className="pt-32 420:pt-64">
          <Button handleClick={handleStop} type={buttonTypes.delete}>
            Stop Timeline
          </Button>
        </div>
      </ViewMoreLess>
